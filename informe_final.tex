...
Para encontrar matemáticamente este límite y definir en qué punto el sistema entra en una condición de falla, realizamos un **balance térmico en estado estacionario** (régimen permanente, cuando el transitorio ha concluido y las variables se han estabilizado). En este estado, la temperatura del aire en el pasillo frío ($T_{out}$) es el resultado del balance estático entre la temperatura base de la sala ($T_0$), el enfriamiento provocado por la apertura del CRAC ($K \cdot U$, donde la ganancia de la planta $K$ es negativa) y la perturbación térmica externa ($D$):
\begin{equation}
    T_{out} = T_0 + K\,U + D
\end{equation}
Para asegurar la estabilidad, deseamos que en estado permanente la temperatura coincida exactamente con nuestro setpoint de consigna ($T_{out} = T_{sp}$). Despejando el esfuerzo de control necesario $U$ obtenemos:
\begin{equation}
    U = \frac{T_0 - T_{sp} + D}{|K|}
\end{equation}
Evaluando con los parámetros nominales del diseño ($K = -0.25$\,\textdegree C/\%, $T_0 = 28$\,\textdegree C y $T_{sp} = 22$\,\textdegree C), la ecuación de esfuerzo se reduce a:
\begin{equation}
    U = 4\,(28 - 22 + D) = 24 + 4D
\end{equation}
