...
A los fines del modelado y para simplificar el desarrollo analítico, la dinámica de los actuadores se representa mediante la función de transferencia $G_a(s)$. Dado que su respuesta es sumamente veloz frente a la constante de tiempo térmica de la sala ($\tau = 35$\,s), se considera un comportamiento ideal con $G_a(s) = 1$, de modo que $U(s) = U_{PID}(s)$.
...
\begin{figure*}[!t]
    ...
    \begin{tikzpicture}[auto, >=stealth, scale=0.85, transform shape]
        ...
        \node [block, right=0.8cm of sum_pid] (actuator) {Actuador \\ $G_a(s)$};
    ...
    \shorthandon{<}
    \shorthandon{>}
    \caption{Diagrama de bloques del lazo de control de temperatura del Data Center. Se detalla la señal de referencia $R(s)$, el transmisor de referencia $H_r(s)$, el error $E(s)$, el controlador PID $G_c(s)$, el actuador $G_a(s)$, la planta térmica $G_p(s)$, la perturbación $D(s)$ y el sensor de realimentación $H(s)$.}
    \label{fig:bloques}
\end{figure*}
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[auto, >=stealth, scale=0.8, transform shape]
        ...
        % Actuator
        \node [block, right=0.6cm of gc] (ga) {$G_a(s)$};
...
Si implementáramos el lazo con una realimentación negativa convencional ($E = V_{sp} - V_{sensor}$) utilizando ganancias del controlador positivas ($G_c(s) > 0$), el producto de lazo abierto $G_{ol}(s) = G_c(s) G_a(s) G_p(s) H(s)$ tendría un signo negativo neto. Esto transformaría el denominador del lazo cerrado convencional ($1 + G_{ol}(s)$) en un término de la forma $1 - |G_{ol}(s)|$, lo que algebraicamente representa una realimentación constructiva o \textbf{realimentación positiva}.
...
\begin{align}
    G_d(s) &= - G_c(s) G_a(s) G_p(s) \\
    G_{ol}(s) &= G_d(s) H(s) = - G_c(s) G_a(s) G_p(s) H(s)
\end{align}

Esta corrección compensa matemáticamente la ganancia estática negativa del proceso (considerando un comportamiento ideal del actuador con $G_a(s) = 1$), logrando que $G_{ol}(s)$ sea positiva a bajas frecuencias y garantizando que el sistema regule en realimentación negativa. En la Fig. \ref{fig:bloques_simples} se detalla este esquema simplificado del lazo y cómo se definen ambos componentes.
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[auto, >=stealth, scale=0.82, transform shape]
        ...
        \node [block, right=1.3cm of gc] (ga) {$G_a(s)$};
...
El actuador, modelado como se indicó en la Sección~\ref{sec:alcance} con dinámica ideal $G_a(s) = 1$, transfiere esta señal sin modificarla: $U(s) = U_{PID}(s)$.
...
\begin{figure}[htbp]
    ...
    \caption{Respuesta al escalón de referencia ($D = 0$). En el gráfico superior (temperatura) se representa la evolución de la temperatura de salida $T_{out}(t)$ (curva azul sólida) partiendo del estado inicial $T_0 = 28$\,$^\circ$C (línea punteada gris) y convergiendo de forma sobreamortiguada al setpoint de $22$\,$^\circ$C (línea discontinua roja) dentro de la banda de tolerancia de $\pm 2$\,\% ($0.12$\,$^\circ$C, franja sombreada verde). En el gráfico inferior (error) se detalla la señal de error de seguimiento $e(t) = T_{sp} - T_{out}$ (curva verde sólida), la cual inicia en $-6$\,$^\circ$C y alcanza el valor de error nulo con un tiempo de asentamiento $t_s \approx 166$\,s.}
...
\begin{figure}[htbp]
    ...
    \caption{Rechazo de perturbación ($R = 0$). Simulación del efecto de un ataque DoS de $+5$\,$^\circ$C. El gráfico superior muestra la temperatura de salida $T_{out}(t)$ (curva azul sólida), la cual parte del setpoint de $22$\,$^\circ$C (línea discontinua roja), alcanza un pico máximo de $24.36$\,$^\circ$C a los $t = 45$\,s debido a la inercia térmica y regresa progresivamente a la banda de tolerancia de $\pm 2$\,\% (franja sombreada verde). El gráfico inferior muestra la señal de error de regulación $e(t)$ (curva verde sólida); el área sombreada bajo dicha curva representa gráficamente la Severidad de la Excursión Térmica (TES), la cual es anulada en estado estacionario gracias a la acción integral.}
    \label{fig:perturbacion}
\end{figure}

\subsection{Límites Operativos y Condiciones de Falla}

Cualquier sistema físico de control posee restricciones en sus actuadores, ya que no existen fuentes capaces de entregar energía infinita. En nuestro caso, la unidad CRAC tiene una capacidad de enfriamiento delimitada por un rango de operación física de $0$\,\% (unidad apagada o válvulas cerradas) a $100$\,\% (unidad a máxima potencia). 
...
\begin{equation}
    U = \frac{T_0 - T_{sp} + D}{|K|}
\end{equation}
Esta ecuación posee una interpretación física muy directa:
\begin{itemize}
    \item \textbf{El numerador ($T_0 - T_{sp} + D$):} Representa los grados Celsius totales que el CRAC debe extraer del pasillo frío. Está compuesto por la diferencia térmica base para alcanzar el setpoint ($T_0 - T_{sp}$) más el calor adicional aportado por la perturbación ($D$).
    \item \textbf{El denominador ($|K|$):} Representa la ganancia del proceso en valor absoluto (cuántos grados Celsius disminuye la temperatura por cada 1\% de apertura del CRAC).
\end{itemize}
...
Evaluando con los parámetros nominales del diseño ($|K| = 0.25$\,$^\circ$C/\%, $T_0 = 28$\,$^\circ$C y $T_{sp} = 22$\,$^\circ$C), la ecuación de esfuerzo se reduce a:
\begin{equation}
    U = \frac{28 - 22 + D}{0.25} = 4\,(6 + D) = 24 + 4D
\end{equation}
Donde la constante de $24$\,\% representa el esfuerzo base del CRAC necesario para mantener el pasillo a $22$\,$^\circ$C partiendo de los $28$\,$^\circ$C iniciales, en ausencia completa de perturbaciones ($D = 0$).

\textbf{Condición de operación normal.} El sistema operará bajo control cerrado (operación normal) siempre y cuando el CRAC no se sature físicamente, lo cual exige que el esfuerzo calculado no supere el $100$\,\%. Resolviendo la inecuación:
\begin{equation}
    24 + 4D \le 100 \quad\Longrightarrow\quad D \le 19\,^\circ\text{C}
\end{equation}
Para el caso general con temperaturas arbitrarias, el límite admisible de perturbación es $D \le 25 - (T_0 - T_{sp})$. Si la perturbación $D$ supera este umbral (por ejemplo, $D = 25$\,$^\circ$C), el CRAC operará saturado al $100$\,\% de su potencia y la temperatura del pasillo frío quedará permanentemente desviada por encima del setpoint.

\textbf{Definición de falla.} A partir de este límite físico, el sistema clasifica las condiciones de falla del lazo de control según dos criterios:
\begin{itemize}
    \item \textbf{Falla por Capacidad de Enfriamiento (Rojo):} Ocurre cuando la perturbación excede el límite teórico ($D > 19$\,$^\circ$C) haciendo que el CRAC se sature al $100$\,\% por un tiempo continuo superior a $t_{\text{sat}} \ge 30$\,segundos, lo que rompe el lazo de control y eleva la temperatura ambiente.
    \item \textbf{Falla Térmica Crítica (Rojo):} Se activa si la temperatura física del aire supera los $32$\,$^\circ$C (límite absoluto de ASHRAE), poniendo en riesgo inmediato de daño térmico a los procesadores de los servidores.
\end{itemize}
...
\begin{figure}[htbp]
    ...
    \caption{Respuesta completa y superpuesta del sistema. El gráfico superior detalla la temperatura medida $T_{out}(t)$ (curva azul sólida) en el pasillo frío. Se observa el seguimiento inicial desde $T_0 = 28$\,$^\circ$C (convergencia a la franja verde de tolerancia en $t_s \approx 166$\,s) y la inyección del ataque DoS en $t = 200$\,s (línea punteada vertical naranja), lo que eleva transitoriamente la temperatura hasta un pico de $24.37$\,$^\circ$C antes de estabilizarse nuevamente en la zona segura (período bajo perturbación sombreado en color crema). El gráfico inferior representa la señal de error $e(t) = T_{sp} - T_{out}$ (curva verde sólida) con el desvío térmico acumulado sombreado en rosa, mostrando un error nulo en régimen permanente para ambas etapas.}
    \label{fig:temporal}
\end{figure}
...
\section{Apéndice A: Validación y Ensayos en el Simulador de Escritorio}
\label{sec:sim_tiempo_real}

Para complementar el análisis analítico continuo y validar empíricamente las decisiones de diseño del controlador, se realizaron ensayos dinámicos utilizando el simulador interactivo de escritorio (\texttt{app\_escritorio.py}). El simulador resuelve segundo a segundo la ecuación diferencial del sistema térmico, incorporando las no linealidades del actuador (saturación en $0$-$100$\,\%) y evaluando la clasificación de estados operativos de forma dinámica. A continuación, se detallan tres escenarios experimentales representativos que caracterizan el comportamiento del sistema.

\subsection{Caso 1: Perturbación en el Límite de Capacidad \texorpdfstring{($D = 19$\,$^\circ$C)}{D = 19 C}}
En este ensayo, se inyectó una perturbación térmica tipo escalón de magnitud $D = 19$\,$^\circ$C, la cual se corresponde exactamente con el límite teórico de saturación del actuador en estado estacionario calculado en la Sección V-C. 

Como se observa en la Fig.~\ref{fig:sim_falla_capacidad}, al inicio del transitorio la temperatura del pasillo frío ($T_{out}(t)$) experimenta una excursión térmica moderada y es llevada de regreso de forma sobreamortiguada hasta el setpoint de $22$\,$^\circ$C, logrando anular el error permanente. Sin embargo, para sostener este balance, la acción de control ($u(t)$) del CRAC debe operar permanentemente al $100$\,\% de apertura, dejándolo sin margen de reserva ante cualquier desvío adicional. Transcurridos los $30$\,s continuos en este estado, el simulador clasifica la operación como \texttt{FALLA: CRAC SATURADO}, reflejando un estado crítico de vulnerabilidad operativa.

\begin{figure*}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{sim_falla_capacidad.png}
    \caption{Captura del simulador para una perturbación límite de $D = 19$\,$^\circ$C. El sistema estabiliza la temperatura en $22$\,$^\circ$C, pero exige el $100$\,\% del esfuerzo de control de los ventiladores, de modo que el simulador declara una falla por capacidad tras $30$\,s continuos de saturación.}
    \label{fig:sim_falla_capacidad}
\end{figure*}

\subsection{Caso 2: Falla por Capacidad Excedida y Falla Térmica Crítica \texorpdfstring{($D = 29.5$\,$^\circ$C)}{D = 29.5 C}}
Se simuló un ataque DoS severo inyectando una perturbación térmica de $D = 29.5$\,$^\circ$C, un valor que excede la capacidad máxima de enfriamiento del sistema.

La Fig.~\ref{fig:sim_falla_critica} muestra que el actuador CRAC satura de forma inmediata al $100$\,\% intentando enfriar el pasillo sin éxito. Al ser la carga térmica superior a la potencia frigorífica neta, la temperatura del pasillo frío se eleva continuamente hasta superar el umbral de seguridad de los servidores. El simulador detecta la violación del límite de $32$\,$^\circ$C (ASHRAE A1) a los $t \approx 1035$\,s y conmuta el estado del sistema a \texttt{FALLA: TEMP. CRÍTICA}, lo que en una instalación física dispararía una parada de emergencia de los racks para evitar daño térmico permanente en los procesadores.

\begin{figure*}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{sim_falla_critica.png}
    \caption{Captura del simulador ante un DoS extremo de $D = 29.5$\,$^\circ$C. La potencia de enfriamiento resulta insuficiente, provocando que la temperatura del aire supere el límite crítico absoluto de seguridad de $32$\,$^\circ$C.}
    \label{fig:sim_falla_critica}
\end{figure*}

\subsection{Caso 3: Diagnóstico y Resolución del Bug de Anti-Windup}
Durante las fases iniciales de ensayo de la aplicación de escritorio, se inyectó un DoS de magnitud $25$\,$^\circ$C con el fin de evaluar la respuesta en saturación. Se observó un comportamiento anómalo detallado en la Fig.~\ref{fig:sim_bug_saturacion}: la temperatura se estabilizó permanentemente en $28.8$\,$^\circ$C (un error estacionario de $-6.77$\,$^\circ$C), mientras que el esfuerzo de control del CRAC permaneció estancado en un $96.9$\,\%, sin llegar a demandar la potencia total.

El análisis de esta falla reveló que se trataba de un problema en el lazo de anti-windup: el límite de sujeción (clamping) de la acción integral en el código del controlador estaba limitado estáticamente a $\pm 200$. Dado que la ganancia integral es $K_i = 0.4$, la máxima contribución posible de la acción integral al esfuerzo de control era de $0.4 \times (-200) = -80$\,\%, recayendo en el término proporcional el aporte del $20$\,\% restante para alcanzar la saturación. Dado que en régimen permanente la acción proporcional depende del error físico y este tendía a achicarse, el controlador era incapaz de suministrar el esfuerzo total de control. 

Este ensayo validó la decisión de incrementar el límite del integrador a $\pm 500$ (cumpliendo con la condición matemática de $I_{\text{max}} \ge 100/K_i$), lo cual permitió al actuador alcanzar el $100$\,\% de apertura y reducir el error estacionario al menor nivel físicamente posible ante perturbaciones de gran escala.

\begin{figure*}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{sim_bug_saturacion.png}
    \caption{Diagnóstico experimental de la saturación prematura. Debido a un límite de sujeción integral de $\pm 200$, la acción integral se bloqueaba limitando el esfuerzo del CRAC al $96.9$\,\% y dejando un desvío térmico permanente de $-6.77$\,$^\circ$C.}
    \label{fig:sim_bug_saturacion}
\end{figure*}

\clearpage
\section{Apéndice B: Pautas Reglamentarias del Trabajo Final Integrador}
