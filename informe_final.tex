...
En $t=200$\,s, se introduce una perturbación tipo escalón de magnitud $5.0^\circ$C, simulando la carga térmica generada por un pico de procesamiento masivo provocado por un ataque informático de denegación de servicio (DoS). 
...
Más allá de la condición de falla funcional por saturación permanente del actuador, existe un criterio de calidad de servicio (QoS) térmica que antecede a la falla, el cual está regulado por la normativa de la asociación americana \textbf{ASHRAE TC 9.9} para entornos de clase A1 (Data Centers empresariales):
\begin{itemize}
    \item \textbf{Rango Recomendado ($18\text{ }^\circ\text{C} \le T \le 27\text{ }^\circ\text{C}$):} Es el intervalo térmico óptimo para asegurar la máxima confiabilidad del hardware. Superar los $27$\,$^\circ$C degrada la vida útil de los servidores debido a fatiga térmica.
    \item \textbf{Rango Admitido ($15\text{ }^\circ\text{C} \le T \le 32\text{ }^\circ\text{C}$):} Límites extremos de operación segura. Exceder los $32$\,$^\circ$C constituye una falla térmica crítica y desencadena el apagado automático (\textit{thermal shutdown}) de los servidores para proteger la CPU.
\end{itemize}

Para capturar y evaluar cuantitativamente el estrés térmico del sistema ante transitorios y perturbaciones (como el ataque DoS), definimos tres métricas clave de QoS térmica en el simulador:
\begin{enumerate}
    \item \textbf{Tiempo sobre el límite ASHRAE recomendado ($t_{>27}$):}
    \begin{equation}
        t_{>27} = \int_{t_{\text{inicio}}}^{t_{\text{fin}}} I(T(t) > 27) \, dt \quad \text{[s]}
    \end{equation}
    Donde $I(\cdot)$ es la función indicadora. Mide el tiempo de degradación de vida útil.
    \item \textbf{Tiempo sobre el límite de seguridad crítico ($t_{>32}$):}
    \begin{equation}
        t_{>32} = \int_{t_{\text{inicio}}}^{t_{\text{fin}}} I(T(t) > 32) \, dt \quad \text{[s]}
    \end{equation}
    Mide el tiempo de riesgo de daño físico inmediato y pérdida de servicio.
    \item \textbf{TES (Thermal Excursion Severity - Severidad de Excursión Térmica):}
    Representa la acumulación del error de temperatura excedente respecto al setpoint durante el ataque DoS:
    \begin{equation}
        \text{TES} = \int_{t_{\text{inicio}}}^{t_{\text{fin}}} \max(0, T(t) - T_{sp}) \, dt \quad \text{[}^\circ\text{C}\cdot\text{s]}
    \end{equation}
\end{enumerate}

Para reflejar estas normativas en tiempo real, el simulador de escritorio clasifica dinámicamente el estado de operación de la planta evaluando segundo a segundo las variables de estado físicas reales ($T(t)$ y $u(t)$) en lugar de una lógica estática de perturbación:
\begin{itemize}
    \item \textbf{Operación Normal (Verde)}: La temperatura se mantiene dentro del rango seguro ($T(t) \le 27$\,$^\circ$C) y el actuador no está saturado ($t_{\text{sat}} = 0$\,s).
    \item \textbf{Advertencia / Al Límite (Amarillo)}: La temperatura ingresa a la zona de degradación ($27\text{ }^\circ\text{C} < T(t) \le 32\text{ }^\circ\text{C}$), o el actuador se satura de forma temporal o persistente ($t_{\text{sat}} > 0$\,s) pero manteniendo la temperatura bajo control dentro del rango seguro ($T(t) \le 27$\,$^\circ$C).
    \item \textbf{Estado de Falla (Rojo)}: La temperatura excede el límite crítico ($T(t) > 32$\,$^\circ$C, Falla Térmica) o la capacidad de enfriamiento se satura de forma persistente ($t_{\text{sat}} \ge 30$\,s) perdiendo la capacidad de mantener el rango recomendado ($T(t) > 27$\,$^\circ$C, Falla por Capacidad).
\end{itemize}
