...
\begin{itemize}
    \item \textbf{Operación Normal (Verde)}: La temperatura del aire se mantiene segura ($T(t) \le 27$\,$^\circ$C) y el actuador opera holgado ($t_{\text{sat}} < 30$\,s).
    \item \textbf{Advertencia / Al Límite (Amarillo)}: La temperatura ingresa a la zona de degradación ($27\text{ }^\circ\text{C} < T(t) \le 32\text{ }^\circ\text{C}$) o el actuador se satura temporalmente ($U(t) = 100\%$ por $t_{\text{sat}} < 30$\,s) para contener la perturbación.
    \item \textbf{Estado de Falla (Rojo)}: La temperatura excede el límite crítico ($T(t) > 32$\,$^\circ$C) o la capacidad de enfriamiento se satura de forma persistente ($t_{\text{sat}} \ge 30$\,s).
\end{itemize}
